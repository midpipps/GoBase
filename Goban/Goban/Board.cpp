...
Goban::Board::~Board()
{
	for (size_t c = 0; c < intersectionMap.size(); c++)
	{
		...
		{
			delete intersectionMap.at(c).at(r);
		}
	}
}
...
void Goban::Board::createBoard()
{
	...
	for (size_t c = 0; c < intersectionMap.size(); c++)
	{
		for (size_t r = 0; r < intersectionMap.at(c).size(); r++)
		{
			intersectionMap.at(c).at(r) = new Intersection();
...
void Goban::Board::playStone(Player *playerToPlay, size_t r, size_t c)
{
	if (isValidPosition(r, c))
	{
...
bool Goban::Board::isValidPosition(size_t r, size_t c)
{
